...
// État pour les départements (pour le select)
const [departments, setDepartments] = useState([])
  ...
  const fetchUsers = async () => {
    setLoading(true)
    try {
      const response = await axios.get("http://localhost:8000/api/users/ra")
  ...
  // Ajoutez cet appel dans useEffect
  useEffect(() => {
    fetchUsers()
    fetchDepartments()
  }, [])
                    ...
                    {currentUsers.length > 0 ? (
                      currentUsers.map((user, index) => (
                        <tr key={user.id} className={index % 2 === 0 ? (darkMode ? "bg-gray-800" : "bg-gray-50") : ""}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm">{user.numero_badge}</td>
    ...
    <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          <select
            id="role"
            className={`mt-1 block w-full rounded-md shadow-sm ${darkMode ? "bg-gray-700 border-gray-600 text-white" : "bg-white border-gray-300 text-gray-900"} border p-2`}
            value={formData.role}
          ...
          >
            ...
            <option value="Admin">Admin</option>
              ...
              <div>
                <label htmlFor="department_id" className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                  Département
                </label>
                <select
                  id="department_id"
                  className={`mt-1 block w-full rounded-md shadow-sm ${darkMode ? "bg-gray-700 border-gray-600" : "bg-white border-gray-300"} border p-2`}
